...
local function GetNodeIndexByID(tree, nodeID)
    local NodeIDCache = {}
...
local function GetIndexByID_tb(nodeID_tb)
    local tree = getTreeComponentCore()
    local NodeIDCache = {}
    local NodeIndex = {}
...
--------------------得到当前NodeID的上一个父级节点的ID
function module.getCurrentNodeID_Parent()
    local player = module.getMasterPlayerUtils()
    if player == nil then
        return "Error"
    end
    ...
    local myBhvt = player.masterPlayer:call("get_GameObject"):call("getComponent(System.Type)",
        sdk.typeof("via.behaviortree.BehaviorTree"))
    local CurrentNodeID = myBhvt:call("getCurrentNodeID", 0)
    local target_node = getNodeByNodeID(CurrentNodeID)
    local parent_NodeID = target_node:get_parent():get_id()
    if parent_NodeID == 0 then
        return
    end
    target_node = parent_NodeID

    return parent_NodeID
end
--------------------得到当前NodeID以及其的、所有父级节点的ID，返回一个表
function module.getCurrentNodeID_Parent_tb()
    local player = module.getMasterPlayerUtils()
    if player == nil then
        return "Error"
    end
    local parent_NodeID_tb = {}
    local myBhvt = player.masterPlayer:call("get_GameObject"):call("getComponent(System.Type)",
        sdk.typeof("via.behaviortree.BehaviorTree"))
    local CurrentNodeID = myBhvt:call("getCurrentNodeID", 0)
    table.insert(parent_NodeID_tb, CurrentNodeID)
    local target_node = getNodeByNodeID(CurrentNodeID)
    local parent_NodeID
    while parent_NodeID ~= 0 do
        parent_NodeID = target_node:get_parent():get_id()
        table.insert(parent_NodeID_tb, parent_NodeID)
        target_node = getNodeByNodeID(parent_NodeID)
    end
    return parent_NodeID_tb
end
----------------------------
...
function module.jumpToNode(null, node_id)
    module.getMasterPlayerUtils().behaviorTree:call(
        "setCurrentNode(System.UInt64, System.UInt32, via.behaviortree.SetNodeInfo)", node_id, nil, nil)
end
...
---------------------------------检查有没有装备某个技能
function module.check_equip_skill(skill)
...
-- 改变一个已有Condition-state对的State
function module.replaceStartTransition(null, NodeID, ConditionID, ReplacedStateIndex)
    local layer
    local tree
    local playercomp = (module.getMasterPlayerUtils()).playerGameObj
    local motion_fsm2 = playercomp:call("getComponent(System.Type)", sdk.typeof("via.motion.MotionFsm2"))
    ...
    if playercomp ~= nil then
        if motion_fsm2 ~= nil then
            ...
            if layer ~= nil then
                ...
                local node_data = node:get_data()
                local transition_array = node_data:get_start_transitions()
                local node_array = node_data:get_start_states()
                for index = 0, node_array:size() - 1 do
                    if tonumber(node_array[index]) == ReplacedStateIndex then
                        if transition_array[index] == ConditionID then
                            isCollision = true
...
-----------------删除指定conditionpairs
function module.eraseConditionPairs(self, NodeID, ConditionID, transitionStateIndex, whetherAddDefaultEvent)
    local layer
    local tree
    local playercomp = (module.getMasterPlayerUtils()).playerGameObj
    local motion_fsm2 = playercomp:call("getComponent(System.Type)", sdk.typeof("via.motion.MotionFsm2"))
    ...
    local tmpNode = tree:get_nodes()[1]
    local tmpEvent = tmpNode:get_data():get_transition_events()
    events:erase(tmpEvent[0])
end

-----------------------------------------删掉指定节点的所有派生和条件
function module.eraseNodePairs(self, NodeID, whetherAddDefaultEvent)
    local layer
    local tree
    local playercomp = (module.getMasterPlayerUtils()).playerGameObj
    local motion_fsm2 = playercomp:call("getComponent(System.Type)", sdk.typeof("via.motion.MotionFsm2"))
    ...
    -- for index, value in ipairs(transition_array:get_elements()) do
    -- 	if tonumber(value) == ConditionID then
    -- 		isCollision = trueweapontype
    -- 	end
    -- end

    for i = 0, transition_array:size() - 1 do
        if tonumber(transition_array[i]) ~= nil then
            transition_array:erase(i)
            node_array:erase(i)
        end
    end
...
function module.getFrame()
    ...
    local Frame = treeLayer:get_Frame()
    if not Frame then
        return
    end
    return Frame
...
function module.getEndFrame()
    local playerBase = module.getMasterPlayerUtils().masterPlayer
    if not playerBase then
        return
    end
    local treeLayer = playerBase:call("getMotionLayer", 0)
    local Frame = treeLayer:get_EndFrame()
    if not Frame then
        return
    end
    return Frame
...
-- 移除一个node中conditon
function module.eraseCondition(null, NodeID, ConditionIndex)
    local treeObj = getTreeComponentCore()
    local node = treeObj:get_node_by_id(NodeID)
    local node_data = node:get_data()
    local conditionInNode = node_data:get_transition_conditions()
    for i = 0, conditionInNode:size() - 1 do
        if conditionInNode[i] == ConditionIndex then
            conditionInNode:erase(i)
        end
    end
end
-- 返回一个指定Node里的一个指定派生节点的一个指定Field的值
function module.checkConditionField(null, NodeID, StationID, FieldName)
    local treeObj = getTreeComponentCore()
    local node = treeObj:get_node_by_id(NodeID)
    local node_data = node:get_data()
    local transition_array = node_data:get_transition_conditions()
    ...
    for i = 0, node_array:size() do
        if node_array[i] == StationID then
            local object_conditon = condition_array[transition_array[i]]
            return object_conditon:get_field(FieldName)
        end
    end
    return false
end
...
function module.checkStation(null, NodeID, StationID)
    local treeObj = getTreeComponentCore()
    local node = treeObj:get_node_by_id(NodeID)
    local node_data = node:get_data()
    local transition_array = node_data:get_transition_conditions()
...
---------得到所有node中含有该特定state的NodeID，返回一个table。建议导出json，否则贼卡。
function module.get_TargetState_nodeid(null, stateIndex)
    local tree = getTreeComponentCore()
    local nodes = tree:get_nodes()
    local tb_node = {}
    for i = 0, nodes:size() - 1 do
        ...
        for j = 0, states:size() - 1 do
            if tonumber(states[j]) == stateIndex then
                table.insert(tb_node, node_id)
            end
        end
    end
    return tb_node
end
-----删除所有node中含有该特定state的派生对
function module.erase_TargetState_nodeid(null, stateIndex)
    local tree = getTreeComponentCore()
    local nodes = tree:get_nodes()
    for i = 0, nodes:size() - 1 do
        local node_data = nodes[i]:get_data()
...
function module.replaceAllStartState_SpecificState(null, originalState, targetState, blackList)
    if blackList == nil then
        blackList = {}
    end
    local tree = getTreeComponentCore()
    ...
    for i = 0, nodes:size() - 1 do
        ...
        if not checkValueExistence(blackList, node_id) then
            local node_data = nodes[i]:get_data()
            local states = node_data:get_start_states()
            local nodeObj = nodes[i]
            for j = 0, states:size() - 1 do
                if tonumber(states[j]) == originalState then
                    states[j] = tonumber(targetState)
    ...
    for i = 0, nodes:size() - 1 do
        ...
        for i = 0, states:size() - 1 do
            if tonumber(states[i]) == stateIndex then
                ...
                table.insert(tbl, conditionIndex)
            end
        end
    end
    return tbl
...
function module.onTerminateVM(null, modName)
    re.on_application_entry("TerminateVM", function()
        json.dump_file(modName, {
            ["off"] = true
        })
    end)
    re.on_application_entry("TerminateHID", function()
        json.dump_file(modName, {
            ["off"] = true
        })
    end)
    re.on_application_entry("TerminateJobScheduler", function()
        json.dump_file(modName, {
            ["off"] = true
        })
    end)
